...
function App(props: any) {
  ...
  const handleSearch = async (qry: string | undefined) => {
    ...
    const q = qry.trim();
    if (isAddress(q)) {
      history.push(`${process.env.PUBLIC_URL}/address/${q}`);
    }
    if (isKeccakHash(q)) {
      let transaction;
      ...
      if (transaction) {
        history.push(`${process.env.PUBLIC_URL}/tx/${q}`);
      }
      let block;
      try {
      ...
      if (block) {
        history.push(`${process.env.PUBLIC_URL}/block/${q}`);
      }
    }
    if (isBlockNumber(q)) {
      ...
      if (block) {
        history.push(`${process.env.PUBLIC_URL}/block/${block.hash}`);
      }
    }
  };
              ...
              <Grid item style={{ marginTop: "8px" }}>
                <Link
                  component={({
                    ...
                    <RouterLink className={className} to={process.env.PUBLIC_URL}>
                      {children}
                    </RouterLink>
                  )}
                >
        ...
        <div style={{ margin: "0px 25px 0px 25px" }}>
          <QueryParamProvider ReactRouterRoute={Route}>
            ...
            <Switch>
              ...
              <Route
                path={process.env.PUBLIC_URL + "/stats/miners"}
                component={MinerStatsPage}
                exact={true}
              />
              <Route path={process.env.PUBLIC_URL + "/stats/miners/:block"} component={MinerStatsPage} />
              <Route path={process.env.PUBLIC_URL + "/block/:hash/raw"} component={BlockRawContainer} />
              <Route path={process.env.PUBLIC_URL + "/block/:hash"} component={Block} />
              <Route path={process.env.PUBLIC_URL + "/blocks/:number"} component={NodeView} />
              <Route
                path={process.env.PUBLIC_URL + "/tx/:hash/raw"}
                component={TransactionRawContainer}
              />
              <Route path={process.env.PUBLIC_URL + "/tx/:hash"} component={Transaction} />
              <Route path={process.env.PUBLIC_URL + "/address/:address/:block"} component={Address} />
              <Route path={process.env.PUBLIC_URL + "/address/:address"} component={Address} />
            </Switch>
          </QueryParamProvider>
        </div>
      </ThemeProvider>
