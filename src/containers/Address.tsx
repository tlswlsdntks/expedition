...
const Address: React.FC<IProps> = ({ match, history }) => {
  ...
  React.useEffect(() => {
    ...
    if (blockNum > blockNumber) {
      history.push(`${process.env.PUBLIC_URL}/address/${address}/${blockNumber}`);
    }
    if (blockNum < 0) {
      history.push(`${process.env.PUBLIC_URL}/address/${address}/0`);
    }
    ...
    <>
      ...
      <AddressTransactions
        ...
        onPrev={() => {
          ...
          history.push(`${process.env.PUBLIC_URL}/address/${address}/${newQuery}`);
        }}
        onNext={() => {
